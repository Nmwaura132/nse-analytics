...
class TelegramNotifier:
    ...
    async def send_message(self, message, msg_type="info"):
        """
        Sends a message to Telegram and syncs with the Dashboard.
        """
        # 1. Sync with Dashboard
        try:
            import httpx
            async with httpx.AsyncClient() as client:
                await client.post("http://127.0.0.1:5001/api/notifications", 
                                 json={"message": message, "type": msg_type},
                                 timeout=2.0)
                logger.info("Dashboard notification synced.")
        except Exception as e:
            logger.warning(f"Failed to sync notification to dashboard: {e}")
